...
instance (Num a) => Semigroup (Shape a) where
    Circle r1 <> Circle r2 = Circle (r1 + r2)
    Rect w1 h1 <> Rect w2 h2 = Rect (w1 + w2) (h1 + h2)
    EquiTri r1 <> EquiTri r2 = EquiTri (r1 + r2)
    Circle r <> Rect w h = Circle (r + w + h)
    Circle r <> EquiTri r' = Circle (r + r')
    Rect w h <> Circle r = Circle (r + w + h)
    Rect w h <> EquiTri r = Circle (r + w + h)
    EquiTri r <> Circle r' = Circle (r + r')
    EquiTri r <> Rect w h = Circle (r + w + h)

instance (Num a) => Monoid (Shape a) where
    mempty = Circle 0